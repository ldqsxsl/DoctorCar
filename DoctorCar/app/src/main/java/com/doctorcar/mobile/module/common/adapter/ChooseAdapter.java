package com.doctorcar.mobile.module.common.adapter;
...
public class ChooseAdapter extends RecyclerView.Adapter<ChooseAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(ViewHolder viewHolder, int i) {
        if (i==list.size()-1){
            viewHolder.mImageView.setImageResource(R.drawable.ic_add_a_photo_black_60dp);
        }else {
            PhotoEntry entry = list.get(i);
            Glide.with(mContext)
                    .load(new File(entry.getPath()))
                    .centerCrop()
                    .placeholder(R.drawable.ic_add_a_photo_black_60dp)
