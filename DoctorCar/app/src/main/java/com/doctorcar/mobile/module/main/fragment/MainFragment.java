package com.doctorcar.mobile.module.main.fragment;
...
import com.doctorcar.mobile.DemoAdapter;
import com.doctorcar.mobile.DemoFragment;
import com.doctorcar.mobile.R;
import com.doctorcar.mobile.RecyclerViewAdapter;
import com.doctorcar.mobile.app.CarApplication;
...
import com.doctorcar.mobile.module.blog.activity.SearchBlogActivity;
...
import com.doctorcar.mobile.module.blog.fragment.SelectBlogFragment;
...
import com.doctorcar.mobile.module.mine.activity.PersonalMessageActivity;
import com.doctorcar.mobile.utils.GlideImageLoader;
import com.doctorcar.mobile.utils.ScreenUtils;
import com.doctorcar.mobile.utils.TLUtil;
import com.doctorcar.mobile.utils.UserUtils;
import com.doctorcar.mobile.view.BetterRecyclerView;
import com.doctorcar.mobile.view.banner.Banner;
import com.doctorcar.mobile.view.banner.BannerConfig;
import com.doctorcar.mobile.view.banner.listener.OnBannerClickListener;
...
public class MainFragment  extends Fragment implements OnBannerClickListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        if (arguments == 0) {
            ...
            banner.setLayoutParams(new LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, ScreenUtils.getScreenH(getActivity())/5));
            //简单使用
            banner.setImages(CarApplication.images)
...
//            initData();
    ...
    public  void initRecyclerView(RecyclerView recyclerView){
        recyclerView.setLayoutManager(new LinearLayoutManager(getActivity(), LinearLayoutManager.VERTICAL, false));// 布局管理器。
    ...
    //初始化
    public void initNewAsk(View view){
        RecyclerView newAskRv = (RecyclerView) view.findViewById(R.id.home_fg_new_ask_rv);
        initRecyclerView(newAskRv);
        ProblemBean problemBean = new ProblemBean();
        problemBean.setProblem_title("sdsdsdsds");
        problemBeanList.add(problemBean);
        NewAskAdapter newAskAdapter = new NewAskAdapter(getActivity(),problemBeanList);
        newAskRv.setAdapter(newAskAdapter);
    }

    public void initArticle(View view){
        RecyclerView articleRv = (RecyclerView) view.findViewById(R.id.home_fg_article_rv);
        initRecyclerView(articleRv);
        ProblemBean problemBean = new ProblemBean();
        problemBean.setProblem_title("fdkopdofkop");
        ArticleAdapter articleAdapter = new ArticleAdapter(getActivity(),problemBeanList);
        articleRv.setAdapter(articleAdapter);
    }

    public void initAnswerStar(View view){
...
//        recyclerView.addItemDecoration(dividerItemDecoration);// 添加分割线。
    ...
    public void initBlogView(View view) {
            ...
            public boolean onMenuItemClick(MenuItem item) {
                switch (item.getItemId()) {
                    case R.id.blog_search:
                        startActivity(new Intent(getActivity(),SearchBlogActivity.class));
        ...
        easyIndicator.setTabTitles(new String[]{"精选博客","我的文章", "草稿箱"});

        // 自定义设置
        easyIndicator.setViewPage(viewPager, new FragmentAdapter(getFragmentManager(),
                new Fragment[]{new SelectBlogFragment(),new MineArticlesFragment(), new DraftBoxFragment()}));
...
//		});
    ...
    public abstract class AppBarStateChangeListener implements AppBarLayout.OnOffsetChangedListener {
        ...
        private DemoFragment.State mCurrentState = DemoFragment.State.IDLE;

        @Override
        public final void onOffsetChanged(AppBarLayout appBarLayout, int i) {
            if (i == 0) {
                if (mCurrentState != DemoFragment.State.EXPANDED) {
                    onStateChanged(appBarLayout, DemoFragment.State.EXPANDED);
                }
                mCurrentState = DemoFragment.State.EXPANDED;
            } else if (Math.abs(i) >= appBarLayout.getTotalScrollRange()) {
                if (mCurrentState != DemoFragment.State.COLLAPSED) {
                    onStateChanged(appBarLayout, DemoFragment.State.COLLAPSED);
                }
                mCurrentState = DemoFragment.State.COLLAPSED;
            } else {
                if (mCurrentState != DemoFragment.State.IDLE) {
                    onStateChanged(appBarLayout, DemoFragment.State.IDLE);
                }
                mCurrentState = DemoFragment.State.IDLE;
            }
        }

        public abstract void onStateChanged(AppBarLayout appBarLayout, DemoFragment.State state);
    }
    ...
    private void initDemoMine(View view) {
        ...
        appBarLayout.addOnOffsetChangedListener(new MainFragment.AppBarStateChangeListener() {
            @Override
            public void onStateChanged(AppBarLayout appBarLayout, DemoFragment.State state) {
                Log.d("STATE", state.name());
                TextView textView = (TextView) toolbar.getMenu().getItem(0).getActionView();
                if (state == DemoFragment.State.EXPANDED) {
                    ...
                    //展开状态
                } else if (state == DemoFragment.State.COLLAPSED) {
...
//		});
    ...
    public void OnBannerClick(int position) {
        TLUtil.showToast("你点击了"+position);
    }
